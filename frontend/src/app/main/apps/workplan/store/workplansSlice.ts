...
export const getWorkplansForRangeDate = createAppAsyncThunk<WorkplansType, { startDate: string; endDate: string }>(
	'workplansApp/workplans/getWorkplansForRangeDate',
	async ({ startDate, endDate }) => {
		const response = await axios.get(`/api/workplans?startDate=${startDate}&endDate=${endDate}`);
		// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
		const data = (await response.data) as WorkplansType;
